...
from .engine import BlackwellEngine, MODEL_ID, REVISION, hardware_info, model_path
from .workloads import workload

BACKENDS = ("upstream", "upstream-fast", "upstream-compile", "eager", "fused", "fp8")
...
def summarize(samples):
    return {"mean_ms": float(np.mean(samples)), "p50_ms": float(np.percentile(samples, 50)),
            "p95_ms": float(np.percentile(samples, 95)), "min_ms": float(min(samples))}


def run(iterations=50, warmup=5, backends=("upstream", "fused"), cases=None):
    ...
    cases = cases or [(1, "short"), (4, "short"), (16, "short"), (1, "medium"),
                      (4, "medium"), (1, "long"), (16, "long")]
    device = "cuda:0"
    report = {"created_utc": datetime.now(timezone.utc).isoformat(), "hardware": hardware_info(device),
              "packages": {p: version(p) for p in ("torch", "triton", "transformers", "laya", "huggingface-hub", "tokenizers")},
              "model": MODEL_ID, "revision": REVISION, "iterations": iterations, "warmup": warmup,
              ...
              "timing_contract": {"both_backends_same_device": True,
    ...
    report["nvidia_smi"] = subprocess.check_output([
        "nvidia-smi", "--query-gpu=name,driver_version,pstate,temperature.gpu,memory.used,utilization.gpu",
    ...
    for backend in backends:
        ...
        for batch, length in cases:
            ...
            if backend.startswith("upstream") and engine.device.type != "cuda":
                raise RuntimeError("Upstream fell back to CPU; this is not a valid GPU baseline")
            ...
            metrics.update({"backend": backend, "questions": batch, "state_length": length,
                            "input_tokens": response["usage"]["input_tokens"],
                            "decisions_per_second": batch * 1000 / metrics["mean_ms"],
                            "input_tokens_per_second": response["usage"]["input_tokens"] * 1000 / metrics["mean_ms"],
                            "first_request_ms": first_request_ms, "load_ms": load_ms,
            ...
            print(f"{backend:12} q={batch:2} {length:6} p50={metrics['p50_ms']:.3f}ms p95={metrics['p95_ms']:.3f}ms decisions/s={metrics['decisions_per_second']:.1f}", flush=True)
...
def main():
    ...
    report = run(args.iterations, args.warmup, args.backends)
