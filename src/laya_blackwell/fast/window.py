...
def install_window(engine):
    ...
    if model.net.encoder.config.local_attention != 128:
        raise ValueError(
            "Fast mode is validated for the checkpoint local_attention=128"
        )
    if model.net.encoder.layers[0].attn.Wqkv.weight.dtype != torch.bfloat16:
        raise ValueError("Fast mode requires BF16 matrix weights")
    ...
    if original not in source:
        raise RuntimeError(
            "FastDecisionModel changed; review the fast source transformation"
        )
