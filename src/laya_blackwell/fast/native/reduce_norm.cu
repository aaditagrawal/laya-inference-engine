...
std::vector<at::Tensor> fused_norm_vector(at::Tensor x, c10::optional<at::Tensor> r, at::Tensor weight,
 ...
 if(r.has_value()) TORCH_CHECK(r->device()==x.device() && r->numel()==4*x.numel() && r->is_contiguous() && (r->scalar_type()==at::kBFloat16),"Four contiguous BF16 partial matrices required");
...
TORCH_LIBRARY(laya_fast_reduce_norm,m) {
