...
PYBIND11_MODULE(laya_fast_format, m) {
  if (_import_array() < 0) throw py::error_already_set();
  if (_import_umath() < 0) throw py::error_already_set();
  py::class_<FastFormatter>(m, "Formatter")
    .def(py::init<>()).def("__call__", &FastFormatter::call)
    .def("inspect_math", &FastFormatter::inspect_math);
}
