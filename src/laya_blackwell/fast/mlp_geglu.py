...
from .norm import gelu_corrections, gelu_lut
...
@torch.library.custom_op("laya_fast_mlp::geglu", mutates_args=(), device_types="cuda")
...
def install(model, config):
    ...
    for layer in model.net.encoder.layers:
        ...
        if module.bias is not None:
            raise ValueError("Fused MLP requires bias-free input projections")
    gelu_corrections()
    if config[-1]:
        gelu_lut()
    ...
    source = source.replace(before, "fast_mlp_geglu(n, layer.mlp.Wi)")
    namespace = dict(original.__globals__)
    baseline_geglu = namespace["geglu_fn"]

    def selected_geglu(x, module):
        if x.numel() == 64 * 1024 and x.is_contiguous():
            return compiled_project(x, module.weight, config)
        return baseline_geglu(module(x))

    namespace["fast_mlp_geglu"] = selected_geglu
