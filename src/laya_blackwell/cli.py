...
def _engine_arguments(parser):
    parser.add_argument("--backend", choices=("fused", "eager", "fp8"), default="fused",
    ...
    parser.add_argument("--revision", help="Model revision; defaults to the engine's pinned checkpoint")
...
def main(argv=None):
    ...
    info = commands.add_parser("info", help="Print the Blackwell GPU and runtime information")
    ...
    serve.add_argument("--api-key", help="Bearer key; otherwise use LAYA_API_KEY if set")
    serve.add_argument("--no-warmup", action="store_true", help="Skip representative startup warmup")
    ...
    if args.command == "predict":
        ...
        with BlackwellEngine(**_engine_options(args)) as engine:
