...
@torch.library.custom_op(
    "laya_frontier_head::linear", mutates_args=(), device_types="cuda"
)
def linear(
    x: torch.Tensor,
    weight: torch.Tensor,
...
def _relu(module, x):
    if (
        x.numel() // x.shape[-1] == getattr(module, "frontier_head_rows", None)
        and x.is_contiguous()
    ):
        return module(x)
...
def _qkv(x, module):
    config = getattr(module, "frontier_head_config", None)
...
def install(model, report_path=Path("results/frontier/head-gemm.json")):
    report = json.loads(Path(report_path).read_text())
    modules = dict(model.named_modules())
    chosen = {}
    for base in report["rows"]:
        if base["variant"] != "cublas":
            continue
        rows = [
            row
        ...
        for name in base["names"]:
            ...
            if name.endswith(".in_proj"):
                ...
                module.frontier_head_config = config
                continue
            module = modules[name]
            original = module.forward
            if best["relu"]:
                module.frontier_head_rows = m
