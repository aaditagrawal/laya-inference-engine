...
def main():
    ...
    directory = Path(".research/frontier-attention").resolve()
    directory.mkdir(parents=True, exist_ok=True)
    source = Path(__file__).with_name("native_attention.cu").resolve()
    ...
    library = directory / "laya_frontier_attention.so"
    start = perf_counter()
    subprocess.run(
        [
            str(cuda / "bin/nvcc"),
