"""Full-request confirmation of pinned fixed-shape global FlashAttention."""
...
from .global_attention_adapter import install
from .holdout import requests


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--configuration", type=int, default=8)
    ...
    # Keep this ablation reusable after the optional constructor flag is selected.
    options.pop("global_attention", None)
    config = args.configuration
    path = args.output or Path(
        "results/frontier/global-attention-full.json"
        if args.include_padding
        else "results/frontier/global-attention-full-unmasked.json"
    )
    report = {
    ...
    with (
        ...
        if args.include_padding:
            report["padding_build"] = json.loads(
