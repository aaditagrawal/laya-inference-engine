...
// See experiments/native/kernels/pytorch-LICENSE and pinned CUTLASS LICENSE.
#include <ATen/ATen.h>
...
TORCH_LIBRARY(laya_frontier_attention_special, m) {
  m.def("forward(Tensor q, Tensor k, Tensor v, int query_tile, bool bias_support, bool special, Tensor? bias=None) -> Tensor");
}
TORCH_LIBRARY_IMPL(laya_frontier_attention_special, CUDA, m) { m.impl("forward", &attention); }
