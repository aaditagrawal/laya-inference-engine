// Adapted from cenconq25/Turbo-Lossless, revision
// 50b72e5520f93cc855f0bd3a0114a7eb97338588, Apache-2.0.
// See turbo_LICENSE and turbo_NOTICE. Modified for exact sparse escape
// replacement before MMA, configurable tile rows, graph-safe descriptors,
// BF16 outputs, split-K rounding, and partial reduction tiles.
/**
...
__device__ __forceinline__ uint16_t escape_value(int row, int col,
    const int32_t* ro, const int32_t* co, const int16_t* cv) {
    int lo=ro[row], hi=ro[row+1];
    while(lo<hi){int mid=(lo+hi)/2; if(co[mid]<col)lo=mid+1;else hi=mid;}
    return (uint16_t)cv[lo];
}
template<bool EXACT>
__device__ __forceinline__ uint32_t exact_pair(uint32_t pair, uint8_t groups,
    int row, int col, int K, const int32_t* ro, const int32_t* co, const int16_t* cv){
    if(col>=K)return 0;
    if constexpr(EXACT){
      if((groups&15)==0)pair=(pair&0xffff0000u)|escape_value(row,col,ro,co,cv);
      if((groups>>4)==0)pair=(pair&0x0000ffffu)|((uint32_t)escape_value(row,col+1,ro,co,cv)<<16);
    }
    return pair;
}
template<bool EXACT>
__device__ __forceinline__ void v3_decode_a(uint32_t a[4],
    ...
    int row_base, int k_base, int K, const int32_t* ro, const int32_t* co, const int16_t* cv)
{
    ...
    a[0]=exact_pair<EXACT>(a[0], (gw0r0>>gh0)&255,row_base+r0,k_base+k0,K,ro,co,cv);
    a[1]=exact_pair<EXACT>(a[1], (gw0r1>>gh0)&255,row_base+r1,k_base+k0,K,ro,co,cv);
    a[2]=exact_pair<EXACT>(a[2], (gw1r0>>gh1)&255,row_base+r0,k_base+k1,K,ro,co,cv);
    a[3]=exact_pair<EXACT>(a[3], (gw1r1>>gh1)&255,row_base+r1,k_base+k1,K,ro,co,cv);
}

template<int V3_TILE_N, int WCT, int TM, bool EXACT>
__launch_bounds__((TM*2), 2)
__global__ void split12_fused_gemm_v3(
    const __grid_constant__ CUtensorMap sm_tma,
    const __grid_constant__ CUtensorMap gr_tma,
    const __grid_constant__ CUtensorMap b_tma,
    int base_exp,
    __nv_bfloat16* __restrict__ C_matrix,
    int M, int K, int N, int out_stride, int split,
    ...
    const int16_t* __restrict__ p_cv, const int16_t* __restrict__ p_wv)
{
    ...
    const int block_m = blockIdx.y * TM, block_n = blockIdx.x * V3_TILE_N;

    extern __shared__ __align__(128) char smem[];
    uint8_t* sm_buf = (uint8_t*)smem;
    uint8_t* gr_buf = sm_buf + TM * V3_TILE_K * 2;
    __nv_bfloat16* B_buf = (__nv_bfloat16*)(gr_buf + TM * V3_TILE_K / 2 * 2);
    constexpr int DSIZE = TM*V3_TILE_K*2 + TM*V3_TILE_K/2*2 + V3_TILE_K*V3_TILE_N*2*2;
    ...
    #define V3_LOAD(mbar_addr, stg, ks) do { \
        if (warp_id == 0 && elect_sync()) { \
            tma_g2s(s32(sm_buf+(stg)*TM*V3_TILE_K), &sm_tma, (int)(ks), block_m, mbar_addr); \
            tma_g2s(s32(gr_buf+(stg)*TM*V3_TILE_K/2), &gr_tma, (int)((ks)/2), block_m, mbar_addr); \
            tma_g2s(s32((uint8_t*)B_buf+(stg)*V3_TILE_K*V3_TILE_N*2), &b_tma, (int)(ks), block_n, mbar_addr); \
            uint32_t tx = TM*V3_TILE_K + TM*V3_TILE_K/2 + V3_TILE_K*V3_TILE_N*2; \
            mbar_expect_tx(mbar_addr, tx); \
        } \
    } while(0)

    int num_k = ((K+63)/64+split-1)/split;
    int k_start = blockIdx.z*num_k*64;
    int phase = 0;
    V3_LOAD(mbar0, 0, k_start);
    ...
    v3_decode_a<EXACT>(a[0], sm_r, gr_r, r0, r1, twg, 0, base_exp, block_m, k_start, K, p_ro, p_co, p_cv);
    #pragma unroll
    for(int n=0;n<WCT;n++){
        int brow = n*8+bg;
    ...
    for (int tk=0; tk < num_k; tk++) {
        ...
        if (tk+1 < num_k)
            V3_LOAD(next_mbar, next_stage, k_start+(tk+1)*V3_TILE_K);

        sm_r=sm_buf+stage*TM*V3_TILE_K;
        gr_r=gr_buf+stage*TM*V3_TILE_K/2;
        B_r=B_buf+stage*V3_TILE_K*V3_TILE_N;

        #pragma unroll
        for (int ks=1; ks<V3_TILE_K/16; ks++) {
            int cur=ks%2, prev=1-cur;
            v3_decode_a<EXACT>(a[cur], sm_r, gr_r, r0, r1, twg, ks*16, base_exp, block_m, k_start+tk*64, K, p_ro, p_co, p_cv);
            #pragma unroll
            for(int n=0;n<WCT;n++){
                int brow = n*8+bg;
        ...
        if (tk+1 < num_k) {
            ...
            sm_r=sm_buf+next_stage*TM*V3_TILE_K;
            gr_r=gr_buf+next_stage*TM*V3_TILE_K/2;
            B_r=B_buf+next_stage*V3_TILE_K*V3_TILE_N;
            v3_decode_a<EXACT>(a[0], sm_r, gr_r, r0, r1, twg, 0, base_exp, block_m, k_start+(tk+1)*64, K, p_ro, p_co, p_cv);
            #pragma unroll
            for(int n=0;n<WCT;n++){
                int brow = n*8+bg;
                b_reg[n][0][0]=B_READ(B_r, brow, bt*4);
    ...
    { __syncthreads();
      ...
      __syncthreads();
      for(int idx=tid;idx<TM*V3_TILE_N;idx+=(TM*2)){
        int m=idx%TM, b=idx/TM;
        int gm=block_m+m, gb=block_n+b;
        if(gm<M&&gb<N) C_matrix[blockIdx.z*N*M+gb*out_stride+gm]=__float2bfloat16_rn(os[m*OP+b]); }
    }
}
