...
// See ../native/kernels/pytorch-LICENSE and the pinned CUTLASS checkout license.
#include <ATen/ATen.h>
...
TORCH_LIBRARY(laya_frontier_attention, m) {
  m.def("forward(Tensor q, Tensor k, Tensor v, Tensor? bias, int query_tile) -> Tensor");
}
TORCH_LIBRARY_IMPL(laya_frontier_attention, CUDA, m) {
