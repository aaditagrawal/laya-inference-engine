// Welford ordering adapted from PyTorch layer_norm_kernel.cu, BSD-3-Clause.
// PyTorch commit 08187d9e0fba026dc8217405802ab5381dc88d90. See pytorch-LICENSE.
#include <ATen/ATen.h>
...
__global__ void norm_kernel(const float* __restrict__ x, const void* __restrict__ residual,
 ...
 float* __restrict__ y, __nv_bfloat16* __restrict__ z, int d, float eps) {
  ...
  for(int j=0;j<2;j++) {
    ...
    for(int k=0;k<4;k++) {
      ...
      if constexpr(HAS_R) {
        float r;
        if constexpr(RFLOAT) r=static_cast<const float*>(residual)[base+col];
        else r=__bfloat162float(static_cast<const __nv_bfloat16*>(residual)[base+col]);
  ...
  for(int offset=2;offset>0;offset/=2) {
    if(lane==0 && warp>=offset && warp<2*offset) {
      int wi=warp-offset;
      sm[2*wi]=w.mean;sm[2*wi+1]=w.var;sm[4+wi]=w.count;
    }
    __syncthreads();
    if(lane==0 && warp<offset) w=combine(w,{sm[2*warp],sm[2*warp+1],sm[4+warp]});
    __syncthreads();
  }
  if(t==0) {sm[0]=w.mean;sm[1]=w.var/float(d);}
  __syncthreads();
  float mean=sm[0], rstd=rsqrtf(sm[1]+eps);
  #pragma unroll
  for(int j=0;j<2;j++) {
    float4 out;
    ...
    for(int k=0;k<4;k++) {
      ...
      float result=weight[col]*(rstd*(value-mean));
      if constexpr(HAS_B) result+=bias[col];
      z[base+col]=__float2bfloat16_rn(result);
    }
    reinterpret_cast<float4*>(y+base)[t+j*128]=out;
  }
}
std::vector<at::Tensor> fused_norm(at::Tensor x, c10::optional<at::Tensor> r, at::Tensor weight,
 ...
 #define LAUNCH(R,RF,B,P) norm_kernel<R,RF,B,P><<<rows,dim3(32,4),0,stream>>>(x.data_ptr<float>(),rp,weight.data_ptr<float>(),bp,y.data_ptr<float>(),reinterpret_cast<__nv_bfloat16*>(z.data_ptr()),1024,eps)
...
TORCH_LIBRARY(laya_native_exp,m) {
 m.def("norm(Tensor x, Tensor? residual, Tensor weight, Tensor? bias, float eps, bool ptx=False) -> Tensor[]");
 m.impl("norm",torch::dispatch(c10::DispatchKey::CUDA,TORCH_FN(fused_norm)));
}
