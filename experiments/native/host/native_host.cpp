...
class Session {
 public:
  Session(uintptr_t host, uintptr_t device, size_t bytes,
...
PYBIND11_MODULE(laya_native_host, m) {
 py::class_<Session>(m,"Session")
  ...
  .def("fill",&Session::fill).def("replay",&Session::replay).def("run",&Session::run);
}
